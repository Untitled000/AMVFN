...
class TrainerEngine(EngineBase):
    ...
    def _train_epoch(self, dataloader, cur_epoch):
        ...
        for idx, data in enumerate(dataloader):
            # 这里图片的规模是（Batch_size, num_images, channel, height, width）
            ...
            with torch.cuda.amp.autocast():
                ...
                for view_type in output:
                    ...
                    if view_type == 'mv_collection':
                        t = targets.type(torch.long).to(self.device)
                    elif view_type == 'single':
                        t = getTargetsSingle(targets).type(torch.long).to(self.device)
                ...
                if self.md_loss is not None:
                    ...
                    mutual_distillation_loss = self.md_loss(output['mv_collection']['logits'], single_view_logits, targets)
